  ...
  try {
    ...
    return casos.map(caso => {
      return {
        ...
        f_notifImputado: new Date(caso._doc.f_notifImputado),
        notifOfendido: caso.notifOfendido,
        f_notifOfendido: new Date(caso._doc.f_notifOfendido),
        desalojo: caso.desalojo,
        cambioDomicilioVict: caso.cambioDomicilioVict,
        medidasProteccion: caso.medidasProteccion,
        f_emisionMedidas: new Date(caso._doc.f_emisionMedidas),
        imputado: persona.bind(this, caso.imputado),
        ofendido: persona.bind(this, caso.ofendido),
        createdAt: new Date(caso._doc.createdAt),
        updatedAt: new Date(caso._doc.updatedAt)
      };
    })
  } catch (err) {
    throw err
  ...
  try {
    ...
    return informes.map(informe => {
      return {
        ...
        f_informe: new Date(informe._doc.f_informe),
        ...
        createdAt: new Date(informe._doc.createdAt),
        updatedAt: new Date(informe._doc.updatedAt)
      };
    })
  } catch (err) {
    throw err
...
const persona = async personaId => {
  try {
    ...
    return {
      ...
      f_nacimiento: new Date(persona._doc.f_nacimiento),
      sexo: persona.sexo,
      edad: persona.edad,
      ...
      createdAt: new Date(persona._doc.createdAt),
      updatedAt: new Date(persona._doc.updatedAt)
    };
  } catch (err) {
    throw err;
  }
    ...
    try {
      ...
      return personas.map(persona => {
        return {
          ...
          nombre: persona.nombre,
          identificacion: persona.identificacion,
          f_nacimiento: new Date(persona._doc.f_nacimiento).toISOString(),
          sexo: persona.sexo,
          edad: persona.edad,
          provincia: persona.provincia,
          canton: persona.canton,
          ...
          createdAt: new Date(persona._doc.createdAt).toISOString(),
          updatedAt: new Date(persona._doc.updatedAt).toISOString()
        };
      });
    } catch (err) {
      console.log(err);
  ...
  crearPersona: async (args, req) => {
    ...
    try {
      ...
      return {
        ...
        f_nacimiento: new Date(result._doc.f_nacimiento).toISOString(),
        sexo: result.sexo,
        edad: result.edad,
